...
void dynamic_number::find_prior(vector<openuber> all)
{
   int c=0;
                all[c-1].num=all[c].op(all[c-1].num,all[c-2].num);
                all.erase(all.begin()+c-2);
}
...
void dynamic_number::equial()
{
    if(letters.size()!=0)
    {
        ...
        {
            if(letters[c]=='A' and letters[c+1]=='n' and letters[c+2]=='s') //Ans save
            {
                all[all.size()-1]->num=Ans;
            }
            ...
            or letters[c]=='6' or letters[c]=='7' or letters[c]=='8' or letters[c]=='9'))
            {
                ...
                {
                    all[all.size()-1]->num=all[all.size()-1]->num*10+temp;
                }
            }
            ...
            if(letters[c]=='+' or letters[c]=='-' or letters[c]=='/' or letters[c]=='*')
            {
                ...
                all[all.size()-1]->num+=_number2/decimal;//mûveletvégzõ alakra hozás
                all.push_back(a); //new something comes

                if(letters[c]=='+')
                {
                function<double(double, double)> ops = plus<double>();
                all[all.size()-1]->op=ops;
                all.push_back(a);//new something comes
                able=all.size()-1;
                }
                if(letters[c]=='-')
                {
                function<double(double, double)> ops = minus<double>();
                all[all.size()-1]->op=ops;
                all.push_back(a);//new something comes
                able=all.size()-1;
                }
                if(letters[c]=='*')
                {
                function<double(double, double)> ops = multiplies<double>();
                all[all.size()-1]->op=ops;
                all.push_back(a);//new something comes
                }
                if(letters[c]=='/')
                {
                function<double(double, double)> ops = divides<double>();
                all[all.size()-1]->op=ops;
                all.push_back(a);//new something comes
                }
            }
...
//        ///box_end///
    ...
    {
        ///hitbox///
        if(/*40-41 zárójelek*/(ev.keycode>=40 and ev.keycode<=43) or (ev.keycode >= 45 and ev.keycode <= 57))
        {letters.insert(letters.begin()+letter,ev.keycode);counter++;letter++;}
